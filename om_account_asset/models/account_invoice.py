...
class AccountInvoiceLine(models.Model):
    ...
    asset_category_id = fields.Many2one('account.asset.category', string="Catégorie d'actif")
    asset_start_date = fields.Date(string="Date de début de l'actif", compute='_get_asset_date', readonly=True, store=True)
    asset_end_date = fields.Date(string="Date de fin de l'actif", compute='_get_asset_date', readonly=True, store=True)
    asset_mrr = fields.Float(string="Revenus mensuels récurrents", compute='_get_asset_date', readonly=True, digits="Account", store=True)
        ...
        for rec in self:
            ...
            if cat:
                if cat.method_number == 0 or cat.method_period == 0:
                    raise UserError(_("Le nombre d'amortissements ou la durée de "
                                      "votre catégorie d'actif ne peut pas être 0."))
