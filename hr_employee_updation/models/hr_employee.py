...
class Enfant(models.Model):
    ...
    employee_id = fields.Many2one(comodel_name='hr.employee', invisible=1, string="Employee",
                                  help='Select corresponding Employee')

    @api.onchange('date_naissance')
    def age_calc(self):
        if self.date_naissance is not False:
            self.age = (datetime.today().date() - datetime.strptime(str(self.date_naissance), '%Y-%m-%d').date()) //timedelta(days=365)
...
class HrEmployee(models.Model):
    ...
    enfant_en_charges = fields.One2many('hr.employee.children', 'employee_id', string='Enfant en charge',
                                        help='Children information')
    ...
    @api.onchange('enfant_en_charges')
    def get_children(self):
        for employee in self:
            employee.children = len(employee.enfant_en_charges)
